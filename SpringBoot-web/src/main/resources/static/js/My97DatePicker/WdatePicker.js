...
/**
 * 设置皮肤和国际化文件加载位置：
 * 在加载WdatePicker.js前，先设置全局变量
 * window.My97DatePicker = {
        basePath:'', // 设置加载皮肤和国际化文件的目录
        skinBasePath:'', // 单独设置加载皮肤的目录，优先于basePath
        langBasePath:'' // 单独设置加载国际化文件的目录，优先于basePath
   }
 *
 */
...
var $dp, WdatePicker;
(function () {
    //TODO:@add by wangjwc begin
    var p = window.My97DatePicker;
    if(null == p || typeof p !== 'object'){
        p = {
            basePath:'',
            skinBasePath:'',
            langBasePath:''
    ...
    window.My97DatePicker = {
        basePath: p.basePath||'', // 设置加载皮肤和国际化文件的目录
        skinBasePath: p.skinBasePath||'', // 单独设置加载皮肤的目录，优先于basePath
        langBasePath: p.langBasePath||'', // 单独设置加载国际化文件的目录，优先于basePath
        getSkinPath:function(basePath){
            var path = this.skinBasePath||this.basePath||basePath||'';

            return path.replace(/\/$/g,'')+'/skin/';
        },
        getLangPath:function(basePath){
            var path = this.langBasePath||this.basePath||basePath||'';

            return path.replace(/\/$/g,'')+'/lang/';
        }
    };
    //TODO:@add by wangjwc end
    ...
    var $ = {
        ...
        $wdate: true,
        $crossFrame: false,//modify@wangjwc
        $preLoad: false,
        $dpPath: "",
    ...
    A = $.$dpPath || J();
    //TODO:modify@wangjwc begin
    //if ($.$wdate)K(A + "skin/WdatePicker.css");
    if ($.$wdate)K(window.My97DatePicker.getSkinPath(A) + "/WdatePicker.css");
    //TODO:modify@wangjwc end
    V = Y;
    if ($.$crossFrame) {
        try {
        ...
        function E(K, J) {
            ...
            function C() {
                 ...
                 * var $ = [
                 "<head><script>",
                 "",
                 "var doc=document, $d, $dp, $cfg=doc.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}",
                ...
                var $ = [
                    ...
                    "doc.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};",
                    "</script><script src=",
                    window.My97DatePicker.getLangPath(A),
                    //"lang/",
                    _.name,
                    ".js charset=", _.charset, "></script>"
                ];
                ...
                for (var C = 0; C < D.length; C++) {
                    if (D[C].name == J.skin) {
                        ...
                        $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + window.My97DatePicker.getSkinPath(A) /*+ "skin/"*/ + D[C].name + "/datepicker.css\" charset=\"" + D[C].charset + "\"/>");
                        //TODO:modify@wangjwc end
                    }
                }
